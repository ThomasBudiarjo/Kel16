(() => {
  ...
  async function deleteAttachmentById(id){const meta=getAttachmentMeta(id);if(!meta)return;if(!canDeleteAttachment(meta)){showToast("You do not have permission to delete this file.");return;}if(!confirm(`Delete ${meta.name}?`))return;await removeFileBlob(meta.blobKey);saveAttachments(getAttachments().filter(x=>x.id!==id));addAudit("File deleted",`${actorLabel()} | ${meta.name} removed from ${meta.recordId||meta.activity}`,{module:meta.activity,actionType:"Deleted",recordId:meta.recordId||meta.activity,previousValue:meta.name,newValue:"Deleted"});renderAttachments();renderMethodAttachments();renderReportAttachmentList();renderSequenceAttachments();renderAssayAttachments();}
  ...
  function showSection(id) {
    ...
    const pageTitles={dashboard:"Dashboard",method:"Analytical Method",validation:"CSV Validation",audit:"Audit Trail",admin:"Administration"};
...
function initEvents() {
    ...
    $("sequenceChooseFilesBtn")?.addEventListener("click",()=>$("sequenceAttachmentInput").click());
    ...
    $("assayChooseFilesBtn")?.addEventListener("click",()=>$("assayAttachmentInput").click());
  ...
  function openEditURSModal(id){
    ...
    showModal(`<h3>Edit ${escapeHtml(u.id)}</h3><label>URS ID</label><input value="${escapeHtml(u.id)}" readonly><label>Requirement <span class="required">*</span></label><textarea id="editURSRequirement">${escapeHtml(u.text)}</textarea><label>Acceptance Criteria <span class="required">*</span></label><textarea id="editURSCriteria">${escapeHtml(u.criteria)}</textarea><div id="editURSError" class="error-message"></div><div class="modal-actions"><button class="secondary close-modal">Cancel</button><button class="primary" id="saveEditedURS">Save Changes</button></div>`);
    $("saveEditedURS")?.addEventListener("click",()=>{const text=$("editURSRequirement").value.trim(),criteria=$("editURSCriteria").value.trim();if(!text||!criteria){$("editURSError").textContent="Requirement dan Acceptance Criteria wajib diisi.";return;}const old={text:u.text,criteria:u.criteria};state.ursOverrides[id]={text,criteria};localStorage.setItem("hplc_urs_overrides",JSON.stringify(state.ursOverrides));addAudit("URS Modified",`${id} | URS definition modified by QC`,{testId:id,previousValue:JSON.stringify(old),newValue:JSON.stringify({text,criteria})});closeModal();renderURS();showToast(`${id} updated.`);});
  }
  function deleteURS(id){
    if(!isQC()){showToast("Hanya QC yang dapat menghapus URS.");return;}const u=getAllURS().find(x=>x.id===id),r=state.urs[id]||{};if(!u)return;if(r.reviewStatus==="Pending QA Review"||r.reviewStatus==="Approved"){showToast("URS yang sudah diajukan atau disetujui QA tidak dapat dihapus.");return;}if(!confirm(`Are you sure you want to delete ${id}?`))return;state.deletedURS=[...new Set([...state.deletedURS,id])];localStorage.setItem("hplc_deleted_urs",JSON.stringify(state.deletedURS));addAudit("URS Deleted",`${id} | URS deleted by QC`,{testId:id,previousValue:"Active",newValue:"Deleted"});renderURS();updateDashboard();showToast(`${id} deleted.`);
  }
  function openAddURSModal(){
    if(!isQC()){showToast("Hanya QC yang dapat menambahkan URS.");return;}
    const id=nextURSId();
    showModal(`<h3>Add User Requirement Specification</h3><label>URS ID</label><input id="newURSId" value="${id}" readonly><label>Requirement <span class="required">*</span></label><textarea id="newURSRequirement" placeholder="Enter the user requirement..."></textarea><label>Acceptance Criteria <span class="required">*</span></label><textarea id="newURSCriteria" placeholder="Enter the acceptance criteria..."></textarea><div id="newURSError" class="error-message"></div><div class="modal-actions"><button class="secondary close-modal">Cancel</button><button class="primary" id="saveNewURS">Save URS</button></div>`);
    $("saveNewURS")?.addEventListener("click",()=>{
      const text=$("newURSRequirement").value.trim(),criteria=$("newURSCriteria").value.trim();
      if(!text||!criteria){$("newURSError").textContent="Requirement dan Acceptance Criteria wajib diisi.";return;}
      const record={id,text,criteria};
      state.customURS.push(record);
      localStorage.setItem("hplc_custom_urs",JSON.stringify(state.customURS));
      state.urs[id]={testResult:"PENDING",status:"PENDING",reviewStatus:"Not Tested"};
      localStorage.setItem("hplc_urs",JSON.stringify(state.urs));
      addAudit("URS created",`${id} | Requirement added by QC`,{testId:id,previousValue:"N/A",newValue:"PENDING"});
      closeModal();renderURS();updateDashboard();showToast(`${id} added.`);
    });
  }

  function handleURSAction(id){
    const r=state.urs[id];
    if(!r?.executedAt){openURSModal(id);return;}
    if(r.reviewStatus==="Pending QA Review"){isQA()?openURSReview(id):submitURSForQA(id);return;}
    if(r.reviewStatus==="Rejected"){isQC()?openURSModal(id):openURSReview(id);return;}
    if(isQC())openURSModal(id);
    else if(isQA())openURSReview(id);
  }

  function openURSModal(id){
    window._ursPendingFiles=[];
    if(!isQC()){showToast("Hanya QC yang dapat menguji URS.");return;}
    const u=getAllURS().find(x=>x.id===id);
    const existing=state.urs[id]||{};
    ...
    showModal(`<h3>${u.id} — URS Test</h3><label>URS ID</label><input value="${escapeHtml(u.id)}" readonly><label>Requirement</label><textarea readonly>${escapeHtml(u.text)}</textarea><label>Acceptance Criteria</label><textarea readonly>${escapeHtml(u.criteria)}</textarea><label>Actual Result <span class="required">*</span></label><textarea id="ursActual" placeholder="Enter the observed actual result...">${escapeHtml(existing.actual||"")}</textarea><label>Supporting Evidence</label><select id="ursEvidence"><option value="">No attachment</option>${evidenceOptions}</select><label>Upload New Evidence</label><input id="ursEvidenceFile" type="file" multiple accept=".pdf,.csv,.xlsx,.docx,.png,.jpg,.jpeg"><div id="ursPendingFiles" class="pending-file-list"></div><label>Test Result <span class="required">*</span></label><select id="ursTestResult"><option value="">Select result</option><option value="PASS" ${existing.testResult==="PASS"?"selected":""}>PASS</option><option value="FAIL" ${existing.testResult==="FAIL"?"selected":""}>FAIL</option></select><small class="muted">QC must explicitly select the observed test result. The system will save the selection exactly as entered.</small><div id="ursError" class="error-message"></div><div class="modal-actions"><button class="secondary close-modal">Cancel</button><button class="primary" id="saveURSTest">Confirm Test</button></div>`);
    ...
    $("saveURSTest")?.addEventListener("click",async()=>{
      const actual=$("ursActual").value.trim(),result=$("ursTestResult").value;let evidence=$("ursEvidence").value;
      if(!actual){$("ursError").textContent="Actual Result wajib diisi.";return;}
      if(!result){$("ursError").textContent="Test Result wajib dipilih: PASS atau FAIL.";return;}
      const files=Array.from(window._ursPendingFiles||$("ursEvidenceFile")?.files||[]);
      const previous=state.urs[id];state.urs[id]={...previous,status:result,testResult:result,reviewStatus:"Not Submitted",actual,evidence,executedBy:actorLabel(),executedAt:now(),qaReview:null};localStorage.setItem("hplc_urs",JSON.stringify(state.urs));addAudit("URS test executed",`${id} | Test ID ${id} | Result ${result}`,{testId:id,previousValue:previous?.testResult||previous?.status||"PENDING",newValue:result});addAudit("URS actual result recorded",`${id} | Actual Result recorded by QC`,{testId:id,newValue:actual});addAudit("URS test result selected",`${id} | QC selected ${result}`,{testId:id,newValue:result});if(files.length){const created=await attachFiles(files,{activity:"URS",recordType:"urs",recordId:id});if(created[0]){state.urs[id].evidence=created[0].name;localStorage.setItem("hplc_urs",JSON.stringify(state.urs));}}closeModal();renderURS();updateDashboard();showToast(`${id}: ${result}`);
    });
  }
  ...
  function renderSequence(){
    ...
    $("sequenceBody").innerHTML=state.sequence.length?state.sequence.map((s,i)=>{
      const methodName=s.method||fallbackMethod, volume=s.volume??fallbackVolume, methodId=s.methodId||getMethodByIdByName(methodName)?.id||"";
      const expectedRT=s.type==="Blank"?"—":"3.45";
      return `<tr><td><input type="checkbox" class="sequence-row-check" data-seq-index="${i}"></td><td>${i+1}</td><td>${escapeHtml(s.sample)}</td><td>${escapeHtml(s.type)}</td><td>${escapeHtml(s.vial)}</td><td><input class="sequence-inline-input" data-seq-field="volume" data-seq-index="${i}" type="number" min="0.1" step="0.1" value="${escapeHtml(String(volume))}"></td><td><select class="sequence-inline-input" data-seq-field="method" data-seq-index="${i}">${methodOptions(methodId,methodName)}</select></td><td>${expectedRT}</td><td><span class="case-status ${s.status==="Completed"?"pass":""}">${escapeHtml(s.status||"Pending")}</span></td><td><button type="button" class="secondary sequence-save" data-seq-index="${i}">Save</button> <button type="button" class="secondary sequence-delete" data-seq-index="${i}">Delete</button></td></tr>`;
    }).join(""):'<tr><td colspan="10" class="muted">No injections added. Click Add Injection to build the sequence.</td></tr>';
    document.querySelectorAll(".sequence-save").forEach(btn=>btn.addEventListener("click",()=>saveSequenceRow(Number(btn.dataset.seqIndex))));
    document.querySelectorAll(".sequence-delete").forEach(btn=>btn.addEventListener("click",()=>deleteSequenceRow(Number(btn.dataset.seqIndex))));
    renderSequenceMeta();
  }
  ...
  function openSequenceModal(){
    if(!canOperate()){showToast("Hanya QC yang dapat membuat sequence.");return;}
    if(!state.methods.length){showToast("Buat dan simpan Method terlebih dahulu di Method & URS.");return;}
    const defaultMethodId=state.methods[0]?.id||"", defaultVolume=Number($("injectionVolume")?.value)||10;
    showModal(`<h3>Add Injection</h3><label>Sample ID <span class="required">*</span></label><input id="seqSample" type="text" placeholder="Enter Sample ID"><label>Type <span class="required">*</span></label><select id="seqType"><option value="">Select type</option><option>Blank</option><option>Standard</option><option>Sample</option></select><label>Vial <span class="required">*</span></label><input id="seqVial" type="text" placeholder="Enter vial number"><label>Method <span class="required">*</span></label><select id="seqMethod">${methodOptions(defaultMethodId,"")}</select><label>Injection Volume (µL) <span class="required">*</span></label><input id="seqVolume" type="number" min="0.1" step="0.1" value="${defaultVolume}" placeholder="Enter injection volume"><div id="seqError" class="error-message"></div><div class="modal-actions"><button class="secondary close-modal">Cancel</button><button class="primary" id="saveSequenceInjection">Save Injection</button></div>`);
    $("saveSequenceInjection")?.addEventListener("click",()=>{
      const sample=$("seqSample").value.trim(),type=$("seqType").value,vial=$("seqVial").value.trim(),methodId=$("seqMethod").value,volume=Number($("seqVolume").value),method=getMethodById(methodId);
      if(!sample||!type||!vial||!method||!Number.isFinite(volume)||volume<=0){$("seqError").textContent="Sample ID, Type, Vial, Method, dan Injection Volume wajib diisi dengan benar.";return;}
      state.sequence.push({sample,type,vial,methodId:method.id,method:method.name,volume,status:"Ready",createdBy:actorLabel(),createdAt:now()});
      localStorage.setItem("hplc_sequence",JSON.stringify(state.sequence));
      addAudit("Sequence updated",`Injection ${sample} (${type}, vial ${vial}, method ${method.name}, volume ${volume} µL) added`);
      closeModal();renderSequence();showToast("Injection berhasil disimpan.");
    });
  }
  ...
  function renderAttachments(){
    ...
    const rows=items.map(f=>`<tr><td><div class="attachment-name-cell"><span class="file-type-icon"><i data-lucide="${fileIconName(f)}"></i></span><div><b>${escapeHtml(f.name)}</b><small>${escapeHtml(fileTypeLabel(f))} · ${formatFileSize(f.size)}</small></div></div></td><td>${escapeHtml(f.activity||"Other")}</td><td>${escapeHtml(f.user||"—")}</td><td>${escapeHtml(f.time||"—")}</td><td><span class="case-status pass">${escapeHtml(f.status||"Uploaded")}</span></td><td><div class="attachment-actions"><button class="secondary" type="button" data-attachment-view="${escapeHtml(f.id)}"><i data-lucide="eye"></i> View</button><button class="secondary" type="button" data-attachment-download="${escapeHtml(f.id)}"><i data-lucide="download"></i></button>${canDeleteAttachment(f)?`<button class="secondary danger-action" type="button" data-attachment-delete="${escapeHtml(f.id)}"><i data-lucide="trash-2"></i></button>`:""}</div></td></tr>`).join("");
    $("attachmentList").innerHTML=items.length?`<div class="table-wrap"><table><thead><tr><th>File Name</th><th>Related Activity</th><th>Uploaded By</th><th>Date/Time</th><th>Status</th><th>Actions</th></tr></thead><tbody>${rows}</tbody></table></div>`:'<div class="muted">No supporting evidence attached.</div>';
    document.querySelectorAll("[data-attachment-view]").forEach(b=>b.addEventListener("click",()=>openStoredAttachment(b.dataset.attachmentView,"view")));
    document.querySelectorAll("[data-attachment-download]").forEach(b=>b.addEventListener("click",()=>openStoredAttachment(b.dataset.attachmentDownload,"download")));
    document.querySelectorAll("[data-attachment-delete]").forEach(b=>b.addEventListener("click",()=>deleteAttachmentById(b.dataset.attachmentDelete)));
    refreshIcons();
  }
  let mainPendingFiles=[];
  function renderMainPending(){const el=$("attachmentPendingFiles");if(!el)return;el.innerHTML=pendingRows(mainPendingFiles)+(mainPendingFiles.length?`<div class="pending-actions"><span class="muted">${mainPendingFiles.length} file(s) selected</span></div>`:"");el.querySelectorAll(".pending-remove").forEach(b=>b.addEventListener("click",()=>{mainPendingFiles.splice(Number(b.dataset.pendingIndex),1);renderMainPending();}));refreshIcons();}
  async function attachMainPending(){if(!(isQC()||currentIsAdmin())){showToast("Only QC or Admin can upload attachments.");return;}const activity=$("attachmentActivity").value;if(!activity){showToast("Select Related Activity first.");return;}if(!mainPendingFiles.length){showToast("Choose at least one file.");return;}const created=await attachFiles(mainPendingFiles,{activity,recordType:"general",recordId:activity});mainPendingFiles=[];$("attachmentInput").value="";$("attachmentActivity").value="";renderMainPending();renderAttachments();showToast(created.length?`${created.length} file(s) attached.`:"No file attached.");}
  ...
  function renderValidationDetail(id){
    ...
    panel.innerHTML = `
      ...
      <div class="validation-detail-actions">
        ${isQC() && !executed ? `<button class="secondary validation-detail-action" data-action="execute" data-id="${escapeHtml(id)}">Execute Test</button>` : ""}
    ...
    panel.querySelectorAll(".validation-detail-action").forEach(btn=>{
      ...
      if(action==="execute" || action==="edit-result") openValidationModal(vid);
  ...
  function openValidationDefinitionModal(id=null){
    ...
    showModal(`<h3>${existing?"Edit":"Add"} Validation Test</h3><label>Test ID *</label><input id="customValId" value="${escapeHtml(existing?.id||"")}" ${existing?"readonly":""} placeholder="e.g. OQ-011"><label>Test Name *</label><input id="customValTitle" value="${escapeHtml(existing?.title||"")}"><label>Validation Type *</label><select id="customValPhase"><option value="IQ" ${existing?.phase==="IQ"?"selected":""}>IQ</option><option value="OQ" ${existing?.phase==="OQ"?"selected":""}>OQ</option><option value="PQ" ${existing?.phase==="PQ"?"selected":""}>PQ</option></select><label>Requirement / Test Objective *</label><textarea id="customValObjective">${escapeHtml(existing?.objective||"")}</textarea><label>Acceptance Criteria *</label><textarea id="customValCriteria">${escapeHtml(existing?.criteria||"")}</textarea><label>Test Input</label><input id="customValInput" value="${escapeHtml(existing?.input||"")}"><div id="customValError" class="error-message"></div><div class="modal-actions"><button class="secondary close-modal">Cancel</button><button class="primary" id="saveCustomVal">Save Test</button></div>`);
    $("saveCustomVal")?.addEventListener("click",()=>{
      ...
      if(!test.id||!test.title||!test.objective||!test.criteria){$("customValError").textContent="Test ID, Test Name, Objective, dan Acceptance Criteria wajib diisi.";return;}
      ...
      if(existing){
        ...
        state.customValidationCases.push(test);saveCustomValidationCases();addAudit("Validation test added",`${test.id} | ${test.title} added by QC`,{testId:test.id,newValue:"Active"});
      }
      closeModal();renderValidation();renderReport();updateDashboard();showToast(`${test.id} saved.`);
    });
  }
  function deleteCustomValidation(id){
    if(!isQC())return;const t=state.customValidationCases.find(x=>x.id===id);if(!t)return;if(!confirm(`Delete validation test ${id}?`))return;
    delete state.validation[id];delete state.validationReviews[id];delete state.validationHistory[id];state.customValidationCases=state.customValidationCases.filter(x=>x.id!==id);saveCustomValidationCases();localStorage.setItem("hplc_validation",JSON.stringify(state.validation));localStorage.setItem("hplc_validation_reviews",JSON.stringify(state.validationReviews));localStorage.setItem("hplc_validation_history",JSON.stringify(state.validationHistory));addAudit("Validation test deleted",`${id} | Test definition deleted by QC`,{testId:id,newValue:"Deleted"});renderValidation();renderReport();updateDashboard();
  }
  function handleValidationAction(id){const r=state.validation[id];if(!r?.executedAt){openValidationModal(id);return;}if(r.reviewStatus==="Pending QA Review"){isQA()?openValidationReview(id):submitValidationForQA(id);return;}if(r.reviewStatus==="Rejected"){isQC()?openValidationModal(id):openValidationReview(id);return;}if(r.reviewStatus==="Not Submitted"&&isQC()){submitValidationForQA(id);return;}if(isQC()&&!r.reviewStatus)openValidationModal(id);else if(isQA())openValidationReview(id);}
  ...
  function deleteValidationCase(id){
    if(!isQC()){showToast("Hanya QC yang dapat menghapus validation test.");return;}const t=getValidationCase(id),r=state.validation[id]||{};if(!t)return;if(r.executedAt||r.reviewStatus){showToast("Validation test yang sudah dieksekusi tidak dapat dihapus.");return;}if(!confirm(`Are you sure you want to delete ${id}?`))return;state.deletedValidationCases=[...new Set([...state.deletedValidationCases,id])];localStorage.setItem("hplc_deleted_validation_cases",JSON.stringify(state.deletedValidationCases));addAudit("Validation test deleted",`${id} | Test definition deleted by QC`,{testId:id,previousValue:"Active",newValue:"Deleted"});closeModal();renderValidation();renderReport();updateDashboard();
  }
  ...
  function renderSequenceAttachments(){const el=$("sequenceAttachmentList");if(!el)return;const items=getAttachments().filter(a=>a.recordType==="sequence"&&a.recordId==="SEQ-001");el.innerHTML=items.length?items.map(a=>`<div class="compact-attachment-row"><span class="file-type-icon"><i data-lucide="${fileIconName(a)}"></i></span><div><b>${escapeHtml(a.name)}</b><small>${escapeHtml(fileTypeLabel(a))} · ${formatFileSize(a.size)}</small></div><button class="secondary" type="button" data-seq-view="${escapeHtml(a.id)}"><i data-lucide="eye"></i></button><button class="secondary" type="button" data-seq-download="${escapeHtml(a.id)}"><i data-lucide="download"></i></button>${canDeleteAttachment(a)?`<button class="secondary danger-action" type="button" data-seq-delete="${escapeHtml(a.id)}"><i data-lucide="trash-2"></i></button>`:""}</div>`).join(""):'<div class="muted">No sequence attachments.</div>';document.querySelectorAll("[data-seq-view]").forEach(b=>b.addEventListener("click",()=>openStoredAttachment(b.dataset.seqView,"view")));document.querySelectorAll("[data-seq-download]").forEach(b=>b.addEventListener("click",()=>openStoredAttachment(b.dataset.seqDownload,"download")));document.querySelectorAll("[data-seq-delete]").forEach(b=>b.addEventListener("click",()=>deleteAttachmentById(b.dataset.seqDelete)));refreshIcons();}
  function renderAssayAttachments(){const el=$("assayAttachmentList");if(!el)return;const items=getAttachments().filter(a=>a.recordType==="assay");el.innerHTML=items.length?items.map(a=>`<div class="compact-attachment-row"><span class="file-type-icon"><i data-lucide="${fileIconName(a)}"></i></span><div><b>${escapeHtml(a.name)}</b><small>${escapeHtml(fileTypeLabel(a))} · ${formatFileSize(a.size)} · ${escapeHtml(a.time)}</small></div><button class="secondary" type="button" data-assay-view="${escapeHtml(a.id)}"><i data-lucide="eye"></i></button><button class="secondary" type="button" data-assay-download="${escapeHtml(a.id)}"><i data-lucide="download"></i></button>${canDeleteAttachment(a)?`<button class="secondary danger-action" type="button" data-assay-delete="${escapeHtml(a.id)}"><i data-lucide="trash-2"></i></button>`:""}</div>`).join(""):'<div class="muted">No assay attachments.</div>';document.querySelectorAll("[data-assay-view]").forEach(b=>b.addEventListener("click",()=>openStoredAttachment(b.dataset.assayView,"view")));document.querySelectorAll("[data-assay-download]").forEach(b=>b.addEventListener("click",()=>openStoredAttachment(b.dataset.assayDownload,"download")));document.querySelectorAll("[data-assay-delete]").forEach(b=>b.addEventListener("click",()=>deleteAttachmentById(b.dataset.assayDelete)));refreshIcons();}
  ...
  function renderReport(){
    ...
    if(body){
      body.innerHTML=executed.length?executed.map(t=>{const r=state.validation[t.id],pass=r.testResult==="PASS",status=pass?"Passed":"Failed";return `<tr><td><input class="report-test-check" type="checkbox" value="${escapeHtml(t.id)}" ${selectedIds.includes(t.id)?"checked":""}></td><td><b>${escapeHtml(t.id)}</b></td><td>${escapeHtml(t.title)}</td><td>${escapeHtml(validationModule(t))}</td><td><span class="report-mini-status ${pass?"pass":"fail"}">${status}</span></td><td>${escapeHtml(r.executedAt||"—")}</td></tr>`;}).join(""):'<tr><td colspan="6" class="muted">No executed tests.</td></tr>';
      body.querySelectorAll(".report-test-check").forEach(cb=>cb.addEventListener("change",()=>{
        ...
        $("reportSelectedCount").textContent=`${ids.length} test(s) selected`;
        renderReportPaper();
      }));
    }
    const count=selectedIds.length;
    if($("reportSelectedCount")) $("reportSelectedCount").textContent=`${count} test(s) selected`;
    renderReportPaper();

    const r=state.report;
    $("prepareReportBtn").disabled=!(isQC()&&reportEligibility()&&(!r||r.status==="DRAFT"||r.status==="Revision Required"||r.status==="REJECTED"));
    $("reviewReportBtn").classList.toggle("hidden",!(isQA()&&r?.status==="SUBMITTED FOR QA REVIEW"));
  }
  ...
  function renderQAValidationReport(){
    ...
    $("report").innerHTML=`
      <div class="qa-report-page">
        ...
        <div class="section-intro qa-report-intro">
          <span class="tag">COMPUTERIZED SYSTEM VALIDATION</span>
          <h2>QA Review &amp; Approval</h2>
          <p>Review the validation report, add comments, and provide electronic signature.</p>
        </div>
  ...
  function renderReportPaper() {
    const cases=getValidationCases();
    const selected=JSON.parse(localStorage.getItem("hplc_report_selected")||"null") || cases.filter(t=>state.validation[t.id]?.executedAt).map(t=>t.id);
    const ids=selected.filter(id=>cases.some(t=>t.id===id));
    const summary=$("paperTestSummary");
    if(summary) summary.innerHTML=ids.map(id=>{const t=cases.find(x=>x.id===id),r=state.validation[id]||{};const st=r.testResult||"PENDING";return `<div class="paper-test-row"><span>${escapeHtml(id)} — ${escapeHtml(t?.title||"Validation Test")}</span><b class="${st.toLowerCase()}">${escapeHtml(st)}</b></div>`;}).join("")||'<div class="muted">No executed tests.</div>';
    if($("paperOverallResult")) $("paperOverallResult").textContent=reportOverallForQA(ids);
    if($("paperReviewedBy")) $("paperReviewedBy").textContent=state.report?.reviewer||"—";
    if($("paperReportDate")) $("paperReportDate").textContent=state.report?.preparedAt||"12 Sep 2026";
    if($("paperPreparedBy")) $("paperPreparedBy").textContent=state.report?.preparedBy||"QC";
  }

  function prepareReport(){
    if(!isQC()){showToast("QC menyiapkan validation report.");return;}if(!reportEligibility()){showToast("Semua IQ/OQ/PQ test wajib PASS sebelum report dapat disubmit.");return;}
    state.report={status:"SUBMITTED FOR QA REVIEW",preparedBy:actorLabel(),preparedAt:now(),reviewer:"—",reviewedAt:"—",comment:"—",signature:null};localStorage.setItem("hplc_report",JSON.stringify(state.report));addAudit("Validation report submitted","QC submitted validation report for QA review");renderReport();showToast("Validation Report submitted for QA review.");
  }
  ...
  function downloadURS(){const rows=getAllURS().map(u=>{const r=state.urs[u.id]||{};return [u.id,u.text,u.criteria,r.actual||"",r.evidence||"",r.testResult||"PENDING",r.reviewStatus||"Not Tested",r.executedBy||"",r.executedAt||""];});const csv=[["URS ID","Requirement","Acceptance Criteria","Actual Result","Supporting Evidence","Test Result","Review Status","Executed By","Executed At"],...rows].map(r=>r.map(csvCell).join(",")).join("\n");downloadText("URS_Summary.csv",csv);addAudit("URS summary downloaded","URS rekap downloaded as CSV");}
  function downloadSequence(){const rows=state.sequence.map((s,i)=>[i+1,s.sample,s.type,s.vial,s.method||"",s.volume??"",s.status||""]);const csv=[["#","Sample ID","Type","Vial","Method","Volume (µL)","Status"],...rows].map(r=>r.map(csvCell).join(",")).join("\n");downloadText("Sequence_Summary.csv",csv);addAudit("Sequence summary downloaded","Sequence rekap downloaded as CSV");}
  function downloadChromatogram(){const data=getChromatograms(),id=$("chromSampleSelect")?.value,d=data[id];if(!d){showToast("No chromatogram data available.");return;}const rows=(d.points||[]).map(p=>[p.rt,p.response]);const peakRows=[["Peak","RT (min)","Area","Area %","Height","Identification"],...(d.peaks||[]).map(p=>[p.peak,p.rt,p.area,p.areaPct,p.height,p.identification||"Sample"])];const csv=[["Injection",id],["Type",d.type],["Method",d.method||""],["Retention Time (min)","UV Response"],...rows,["",""],...peakRows].map(r=>r.map(csvCell).join(",")).join("\n");downloadText(`${id}_Chromatogram.csv`,csv);addAudit("Chromatogram summary downloaded",`${id} chromatogram data and peak results downloaded as CSV`);}
  function resetAllData(){if(!isAdmin()){showToast("Hanya ADMIN yang dapat reset all data.");return;}if(!confirm("Reset ALL application data? This cannot be undone."))return;localStorage.clear();sessionStorage.clear();location.reload();}
  function renderReportPendingFiles(){const el=$("reportPendingFiles");if(!el)return;const files=window._reportPendingFiles||[];el.innerHTML=pendingRows(files)+(files.length?`<div class="pending-actions"><button id="attachReportPendingBtn" class="primary" type="button"><i data-lucide="paperclip"></i> Attach Selected Files</button></div>`:"");el.querySelectorAll(".pending-remove").forEach(b=>b.addEventListener("click",()=>{files.splice(Number(b.dataset.pendingIndex),1);renderReportPendingFiles();}));$("attachReportPendingBtn")?.addEventListener("click",async()=>{const created=await attachFiles(files,{activity:"Validation Report",recordType:"report",recordId:"VR-001"});window._reportPendingFiles=[];renderReportPendingFiles();renderReportAttachmentList();showToast(created.length?`${created.length} report file(s) attached.`:"No file attached.");});refreshIcons();}
  function dashboardRowsByStatus(kind){
    const cases=getValidationCases();
    return cases.filter(t=>{
      const r=state.validation[t.id]||{};
      if(kind==="pending") return !r.executedAt;
      if(kind==="review") return r.reviewStatus==="Pending QA Review";
      if(kind==="approved") return r.reviewStatus==="Approved";
      if(kind==="failed") return r.testResult==="FAIL";
      return true;
    });
  }
  ...
  function initEvents(){
    ...
    $("instrumentSystemCheckBtn")?.addEventListener("click",()=>{addAudit("System check","HPLC-UV system check executed.",{module:"Instrument",actionType:"Executed"});showToast("System check completed: all modules ready.");});
    $("instrumentViewDetailsBtn")?.addEventListener("click",()=>showToast("HPLC-UV-001 · Qualified · Software v1.0.0"));
    $("instrumentQualificationBtn")?.addEventListener("click",()=>showSection("validation"));
    $("instrumentMaintenanceBtn")?.addEventListener("click",()=>showSection("audit"));
    $("instrumentRecentRunsBtn")?.addEventListener("click",()=>showSection("sequence"));
    $("editSequenceBtn")?.addEventListener("click",()=>{if(!canOperate()){showToast("QA hanya dapat melihat sequence.");return;}showToast("Pilih injection pada tabel untuk mengedit data sequence.");});
    $("deleteSequenceBtn")?.addEventListener("click",()=>{if(!canOperate()){showToast("QA hanya dapat melihat sequence.");return;}const checked=[...document.querySelectorAll(".sequence-row-check:checked")];if(!checked.length){showToast("Pilih injection yang ingin dihapus terlebih dahulu.");return;}if(confirm(`Delete ${checked.length} selected injection(s)?`)){checked.map(c=>Number(c.dataset.seqIndex)).sort((a,b)=>b-a).forEach(i=>state.sequence.splice(i,1));saveSequence();renderSequence();addAudit("Sequence injection deleted",`${checked.length} injection(s) deleted from SEQ-001`,{module:"Sequence",actionType:"Deleted",recordId:"SEQ-001"});showToast("Selected injection(s) deleted.");}});
    ...
    $("downloadURSBtn")?.addEventListener("click",downloadURS);$("downloadSequenceBtn")?.addEventListener("click",downloadSequence);$("downloadChromBtn")?.addEventListener("click",downloadChromatogram);$("addValidationBtn")?.addEventListener("click",()=>openValidationDefinitionModal());$("resetAllDataBtn")?.addEventListener("click",resetAllData);
    ...
    $("addSequenceBtn")?.addEventListener("click",openSequenceModal);$("runSequenceBtn")?.addEventListener("click",runSequence);
    ...
    if($("loadSequenceTemplateBtn")) $("loadSequenceTemplateBtn")?.addEventListener("click",()=>showToast("Template sequence tersedia sebagai referensi."));
    if($("importSequenceBtn")) $("importSequenceBtn")?.addEventListener("click",()=>showToast("Gunakan Add Injection untuk menambahkan data sequence."));
    if($("previewSequenceBtn")) $("previewSequenceBtn")?.addEventListener("click",()=>showToast("Preview sequence siap untuk injection yang tersimpan."));
    if($("viewSequenceRunLogBtn")) $("viewSequenceRunLogBtn")?.addEventListener("click",()=>showSection("audit"));
    ...
    $("verifyDetectorBtn")?.addEventListener("click",()=>{if(!canOperate()){showToast("QA tidak dapat mengubah instrument configuration.");return;}const v=$("detectorTestValue").value,r=$("detectorResult");if(isValidWavelength(v)){r.className="result-box pass-box";r.innerHTML=`<b>PASS</b> — ${escapeHtml(v)} nm is within 190–600 nm.`;addAudit("Detector verification",`Wavelength ${v} nm accepted`);}else{r.className="result-box fail-box";r.innerHTML=`<b>FAIL</b> — ${escapeHtml(v||"blank")} nm is outside 190–600 nm.`;addAudit("Detector verification",`Wavelength ${v||"blank"} nm rejected`);}});
    $("calculateBtn")?.addEventListener("click",calculateResult);$("submitDataReviewBtn")?.addEventListener("click",submitDataReview);$("dataReviewQAButton")?.addEventListener("click",openDataReviewQA);$("chromSampleSelect")?.addEventListener("change",renderChromatogram);
    ...
    if($("saveAssayResultBtn"))$("saveAssayResultBtn")?.addEventListener("click",()=>{calculateResult();showToast("Assay result saved.");});
    if($("downloadAssayResultBtn"))$("downloadAssayResultBtn")?.addEventListener("click",()=>{const c=JSON.parse(localStorage.getItem("hplc_calculation")||"null");if(!c){showToast("Calculate the result first.");return;}const text=`HPLC-UV Assay Result\nInjection: ${c.injection||"—"}\nSample concentration: ${Number(c.concentration).toFixed(3)} µg/mL\nAssay: ${Number(c.assay).toFixed(2)}%\nTime: ${c.time}`;const blob=new Blob([text],{type:"text/plain"}),a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="HPLC-UV_Assay_Result.txt";a.click();URL.revokeObjectURL(a.href);addAudit("Assay result downloaded","Analytical assay result exported");});
    $("prepareReportBtn")?.addEventListener("click",prepareReport);$("reviewReportBtn")?.addEventListener("click",reviewReport);
    $("saveReportDraftBtn")?.addEventListener("click",()=>{state.report={...(state.report||{}),status:"DRAFT",preparedBy:actorLabel(),preparedAt:now()};localStorage.setItem("hplc_report",JSON.stringify(state.report));addAudit("Validation report draft saved","QC saved validation report as draft");renderReport();showToast("Validation report saved as draft.");});
    $("previewReportBtn")?.addEventListener("click",()=>{renderReport();showToast("Report preview updated.");});
  }
  ...
  function initFunctionalButtonFallbacks(){
    ...
    document.querySelectorAll(".module-config").forEach((b,i)=>b.addEventListener("click",()=>{
      if(!canOperate()){showToast("QA hanya dapat melihat konfigurasi instrument.");return;}
      const tabs=document.querySelectorAll(".instrument-tab"); if(tabs[i]) tabs[i].click();
      showToast("Instrument configuration siap diedit.");
    }));
    document.querySelectorAll(".instrument-tab").forEach((b,i)=>b.addEventListener("click",()=>{
      document.querySelectorAll(".instrument-tab").forEach(x=>x.classList.remove("active")); b.classList.add("active");
      const configs=["pumpFlow","instrumentInjectionVolume","ovenTemp","uvWavelength"]; const el=$(configs[i]); if(el) el.focus();
    }));
    document.querySelectorAll(".restore-default").forEach(b=>b.addEventListener("click",()=>{
      ...
      if($("pumpFlow")) $("pumpFlow").value="1.0"; if($("pressureValue")) $("pressureValue").textContent="112 bar";
      if($("instrumentInjectionVolume")) $("instrumentInjectionVolume").value="10";
      if($("ovenTemp")) {$("ovenTemp").value="30";$("ovenValue").textContent="30 °C";}
      if($("uvWavelength")) {$("uvWavelength").value="243";$("uvRange").textContent="Valid range: 190–600 nm";$("uvRange").className="module-value";}
      addAudit("Instrument defaults restored","Instrument settings restored to controlled defaults.",{module:"Instrument",actionType:"Modified"}); showToast("Instrument defaults restored.");
    }));
    document.querySelectorAll(".config-actions button").forEach(b=>b.addEventListener("click",()=>{
      ...
      if(!canOperate()){showToast("QA hanya dapat melihat instrument.");return;}
      addAudit("Instrument settings saved","Instrument operating parameters saved.",{module:"Instrument",actionType:"Modified"}); showToast("Instrument settings saved.");
    }));
    document.querySelectorAll(".diagnostic-btn").forEach(b=>b.addEventListener("click",()=>{addAudit("Diagnostic test","Instrument diagnostic test executed.",{module:"Instrument",actionType:"Executed"});showToast("Diagnostic test completed: all modules ready.");}));
